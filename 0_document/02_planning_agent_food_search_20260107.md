...
| `info_count` | int | 0 | 수집된 항목 개수 (기록용) |
| `is_sufficient` | bool | False | 정보 충분 여부 (기록용) |
...
#### 1. `collected_blogs_count` (블로그 수집 완료 여부)

| 값 | 조건 | Agent 행동 |
|---|------|----------|
| < 5 | 진행 중 | 계속 블로그 수집 및 분석 |
| = 5 | 목표 달성 | 다음 장소로 이동 (종합 분석은 나중에) |
| < 5 (검색 결과 부족) | 검색 결과 자체가 부족 | 가능한 개수로 진행 + 사용자 요청 시 추가 조회 가능 |

**특징**: 5개 달성 시 즉시 다음 장소로 이동하여 효율성 확보

#### 2. `info_count` (수집된 정보 항목 개수)

| 값 | 판단 | Agent 행동 |
|---|------|----------|
| ≥ 3 | 충분 | `is_sufficient = True` (기록만, 행동 변경 없음) |
| < 3 | 부족 | `is_sufficient = False` (기록만, 행동 변경 없음) |
| 0 | 정보 없음 | 그대로 진행 (점수는 모두 기본값 500점) |

**특징**: 기록용으로만 사용, Agent 행동 변경 안 함 ("있는 정보로 최선" 철학)
...
1. LLM에게 전달:
   ...
   - 미달 장소들 (이름, 주소, 미달 이유)
   - 통과 장소들 (이름, 주소)
2. LLM이 네이버 지도 검색 쿼리 재작성 (주로 위치 범위 조정)
...
**루프**: 블로그 분석 (개별) → `collected_blogs_count++` → 5개 미만이면 반복

**탈출 조건**:
- `collected_blogs_count = 5` (목표 달성)
...
**루프**: 연관성 평가 → 3개 미만 → LLM 쿼리 재작성 → 지도 API 재검색 → 블로그 분석 → 연관성 재평가
...
| 분기점 | 조건 | 경로 |
|-------|------|------|
| 블로그 수집 완료 | `collected_blogs_count = 5` | → 다음 장소 |
| 5개 장소 수집 완료 | `current_place_index = 5` | → 연관성 평가 |
| 연관성 평가 후 | 통과 ≥ 3개 | → 종합 분석 |
| 연관성 평가 후 | 통과 < 3개, retry_count = 0 | → 재검색 |
